...
fo = open("./profile results/5stage.riscv.out", "r+")
...
while True:
    ...
    if index != -1 :
        #newline = line[index+3:index+14]
        #print line[index+2:index+12]
        #print "Read Line: %s" % (assembly[line[index+5:index+15]])
        if line.find("STALL") == -1:
            s = line[index+2:index+12]
            temp = count[types[assembly[s]]] + 1
            count[types[assembly[s]]] = temp
        else:
            temp = count['halt'] + 1
            count['halt'] = temp
